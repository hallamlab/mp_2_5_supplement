...
```{r, echo=FALSE}
wd <- "~/Dropbox/manuscripts/metapathways_2_5/mp_2_5_supplement/data/"
```

## Load Required R Packages and Data

Data files can be found [here](https://www.dropbox.com/sh/kq0n07qikmh5rs5/AACwmp2cGLhFjiooHObuw-Sma?dl=0), or in the [Oxford Bioinformatics Supplementary Material]().
...
pwys <- read.table("01_jgi_4093112_combined_unique.pwy.txt.gz", header=T, sep="\t")
blast <- read.table("02_4093112_combined_unique.qced.faa.cog.blast.common.txt.gz", comment.char="#", sep="\t", header=FALSE)
last <- read.table("03_4093112_combined_unique.qced.faa.cog.last.common.txt.gz", comment.char="#", sep="\t", header=FALSE)          
ORFs <- read.table("04_rpkm_master_table_ORF_count.txt.gz", sep="\t", header=T)
RPKMs <- read.delim("05_rpkm_master_table_rpkm.txt.gz", sep="\t", header=T)
```
...
# create Figure 1b
set.seed(12345)
samp <- sample(1:nrow(df.m.eval), round(nrow(df.m.eval)*1.0))
df.m.eval.plot <- df.m.eval[samp,]
p2 <- ggplot(df.m.eval.plot, aes(x=BLAST, y=LAST))
p2 <- p2 + geom_point(size=3, alpha=0.05)
...
* Create confidence intervals with new data
* Add to dataframe

```{r}
rpkm_direct_fit <- lm(I(log(RPKMs))~I(log(ORFs)), data=df.measure.sub)
pred_interval <- predict(rpkm_direct_fit, df.measure.sub, interval="prediction", level =0.90)
...
Any questions or comments, please contact <shallam@mail.ubc.ca>.