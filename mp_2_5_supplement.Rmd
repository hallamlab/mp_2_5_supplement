...
p2 <- p2 + geom_point(size=3, alpha=0.005, aes(color=within))
...
# p2 <- p2 + geom_line(data=df.m.eval.pred, aes(x=BLAST, y=value, group=variable), color="#E91F19", size=0.75)
p2
```
...
# p3 <- p3 + geom_line(data=df.measure.sub.pred, aes(x=ORFs, y=value, group=variable), color="#E91F19", size=0.75)
p3
```
